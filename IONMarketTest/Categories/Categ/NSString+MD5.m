...
-(BOOL)judgePassWordLegal{
    ...
    if ([self length] >= 8){
        ...
        NSString * regex = @"^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{8,}$";
...
- (NSString *)changeToRMB {
    
    NSString *resultStr = [NSString stringWithFormat:@"%ld", (long)[self integerValue]/100];
...
- (NSString *)DaXingHaoHandle {
    NSString *qianStr = [self substringToIndex:3];
    NSString *houStr = [self substringFromIndex:self.length-4];
    NSString *showStr = [NSString stringWithFormat:@"%@****%@", qianStr, houStr];
    return showStr;
}
