...
- (void)initValues
{
    ...
    _line = [[UIImageView alloc] initWithFrame:CGRectMake(0, _topScrollView.bottom -1, self.width, 1)];
    ...
    _rootScrollView.backgroundColor = [UIColor orangeColor];
...
//        _topScrollView.frame = CGRectMake(0,hh, self.width, kHeightOfTopScrollView);
        _topScrollView.frame = CGRectMake(0,0, self.width, kHeightOfTopScrollView);
        _line.frame = CGRectMake(0, _topScrollView.bottom -1, self.width, 1);
//        _rootScrollView.frame = CGRectMake(0,_topScrollView.bottom,self.width,self.height - kHeightOfTopScrollView - hh);
        _rootScrollView.frame = CGRectMake(0,_topScrollView.bottom,self.width,self.height - kHeightOfTopScrollView);
...
- (void)buildUIWithViews:(NSArray *)views andClikBlock:(ClikBlock)clikBlock
{
    ...
    if (_width < SCREEN_WIGHT-25) {
        spacing = (SCREEN_WIGHT-25 - _width)/number + spacing;
        _width = SCREEN_WIGHT-25;
    }
...
- (void)createNameButtonsWithNumber:(NSInteger)number
{
    ...
    for (int i = 0; i < _viewArray.count; i++) {
        ...
        if (i == 0) {
            _shadowView.frame = CGRectMake((width-60)/2, kHeightOfTopScrollView -1, 60, 1.5);
            _shadowView.backgroundColor = self.tabItemBannerSelectedColor;
            button.selected = YES;
...
//            [_shadowView setFrame:CGRectMake(button.x,_topScrollView.bottom -1.2, button.width, 1.2)];
            [_shadowView setFrame:CGRectMake(button.x+((button.width-60)/2),_topScrollView.bottom -1, 60, 1.5)];
            
            CGFloat offsetX = 0;
            float width = [UILabel getSizeWithText:button.titleLabel.text andFont:FONT_ArialMT(kFontSizeOfTabButton) andSize:CGSizeMake(0,kFontSizeOfTabButton)].width + 30;
