...
//    [header setImages:idleImages forState:MJRefreshStateIdle];
    ...
    [header setTitle:@"" forState:MJRefreshStateIdle];//普通状态
    [header setTitle:@"刷新中..." forState:MJRefreshStateRefreshing];//刷新中
    [header setTitle:@"下拉刷新" forState:MJRefreshStatePulling];//松开刷新
    header.lastUpdatedTimeLabel.hidden= YES;//如果不隐藏这个会默认 图片在最左边不是在中间
    header.stateLabel.hidden = NO;//显示状态文本
    return header;
}
...
//    hud.bezelView.color = [[UIColor whiteColor] colorWithAlphaComponent:0.8f];
//
...
//    hud.label.font = FONT_ArialMT(14);
//    hud.label.textColor = [UIColor Black_WordColor];
...
//        hud.bezelView.color = [[UIColor whiteColor] colorWithAlphaComponent:0.1f];
//    }
//}
- (void)showAlertTitle:(NSString *)titleString detailsText:(NSString *)detailsString time:(float)time aboutType:(WHShowViewMode)mode  state:(BOOL)isSuccess {
    if (!detailsString.length) {
        [self showAlertTitle:@"" detailsText:@"未输入提示语句" time:2.0 aboutType:WHShowViewMode_Text state:NO];
        return;
    }
    
    
    if (mode == WHShowViewMode_Text) {//文本提示
        if (isSuccess) {
            [EasyShowTextView showSuccessText:detailsString];
        } else {
            [EasyShowTextView showInfoText:detailsString];
        }
    }
    if (mode == WHShowViewMode_Load) {//加载显示
        
        [EasyShowLodingView showLodingText:detailsString];

    }
    if (mode == WHShowViewMode_Alert) {//AlertController
