...
- (void)initDataBase {
    ...
    NSString *message_key = [[NSUserDefaults standardUserDefaults] objectForKey:[NSString stringWithFormat:@"message_%@.db", [UserData currentUser].uid]];
    if (message_key.length) {
    } else {
        //实例化FMDataBase对象
        //NSLog(@"---path:%@",filePath);
        ...
        [[NSUserDefaults standardUserDefaults] setObject:[UserData currentUser].uid forKey:[NSString stringWithFormat:@"message_%@.db", [UserData currentUser].uid]];
    }
